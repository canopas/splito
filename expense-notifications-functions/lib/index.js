...
exports.onExpenseCreated = (0, firestore_2.onDocumentCreated)({ document: 'groups/{groupId}/expenses/{expenseId}' }, async (event) => {
    var _a;
    try {
        const expenseData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
        if (!expenseData) {
...
exports.onExpenseUpdated = (0, firestore_2.onDocumentUpdated)({ document: 'groups/{groupId}/expenses/{expenseId}' }, async (event) => {
    var _a, _b, _c, _d;
    try {
        const oldExpenseData = (_b = (_a = event.data) === null || _a === void 0 ? void 0 : _a.before) === null || _b === void 0 ? void 0 : _b.data(); // Data before the update
        const newExpenseData = (_d = (_c = event.data) === null || _c === void 0 ? void 0 : _c.after) === null || _d === void 0 ? void 0 : _d.data(); // Data after the update
...
exports.onExpenseDeleted = (0, firestore_2.onDocumentDeleted)({ document: 'groups/{groupId}/expenses/{expenseId}' }, async (event) => {
    var _a;
    try {
        const expenseData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
        if (!expenseData) {
...
function generateNotificationMessage(owedAmount) {
    if (owedAmount < 0) {
...
function calculateOwedAmount(expenseData, memberId) {
    const splitTo = expenseData.split_to;
...
function getTotalSplitAmountOf(expenseData, member) {
    const splitTo = expenseData.split_to;
    if (!splitTo.includes(member))
        return 0;
    const splitType = expenseData.split_type;
    const splitData = expenseData.split_data;
...
exports.onTransactionCreated = (0, firestore_2.onDocumentCreated)({ document: 'groups/{groupId}/transactions/{transactionId}' }, async (event) => {
    var _a;
    try {
...
exports.onTransactionUpdated = (0, firestore_2.onDocumentUpdated)({ document: 'groups/{groupId}/transactions/{transactionId}' }, async (event) => {
    var _a, _b;
    try {
        const oldTransactionData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.before.data(); // Data before the update
        const newTransactionData = (_b = event.data) === null || _b === void 0 ? void 0 : _b.after.data(); // Data after the update
...
exports.onTransactionDeleted = (0, firestore_2.onDocumentDeleted)({ document: 'groups/{groupId}/transactions/{transactionId}' }, async (event) => {
    var _a;
    try {
        const deletedTransactionData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
...
exports.onGroupUpdated = (0, firestore_2.onDocumentUpdated)({ document: 'groups/{groupId}' }, async (event) => {
    var _a, _b;
    try {
        const oldGroupData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.before.data(); // Data before the update
        const newGroupData = (_b = event.data) === null || _b === void 0 ? void 0 : _b.after.data(); // Data after the update
        ...
        const oldMembers = oldGroupData.members; // Members before the update
        const newMembers = newGroupData.members; // Members after the update
        const removedMembers = oldMembers.filter((memberId) => !newMembers.includes(memberId)); // Check for removed members
...
exports.onGroupDeleted = (0, firestore_2.onDocumentDeleted)({ document: 'groups/{groupId}' }, async (event) => {
    var _a;
    try {
        const deletedGroupData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
        if (!deletedGroupData) {
...
async function sendNotification(userId, title, body) {
    try {
        const userDoc = await db.collection('users').doc(userId).get();
        const userData = userDoc.data();
        if (userDoc.exists && (userData === null || userData === void 0 ? void 0 : userData.deviceFcmToken)) {
            const fcmToken = userData === null || userData === void 0 ? void 0 : userData.deviceFcmToken;
            const payload = {
                notification: {
                    title,
                    body,
